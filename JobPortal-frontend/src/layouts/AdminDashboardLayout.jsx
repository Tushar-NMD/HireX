...
import {
  FaUser, FaEdit, FaPlus, FaBriefcase, FaList, FaChartLine,
...
const AdminDashboardLayout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      ...
      {/* Sidebar */}
      <div className={`fixed left-0 top-0 h-full w-80 bg-white/95 backdrop-blur-md shadow-2xl transform transition-transform duration-300 z-50 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
        } lg:translate-x-0`}>
        ...
        <div className="p-6 border-b border-gray-200">
          ...
          {/* Admin Badge */}
          {/* <div className="mt-4 inline-flex items-center space-x-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white px-3 py-1 rounded-full text-sm">
          ...
          {sidebarItems.map((item, index) => {
            ...
            return (
              <Link
                ...
                to={item.path}
                onClick={() => setIsSidebarOpen(false)}
                className={`group flex items-center space-x-3 p-4 rounded-xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${isActive(item.path)
                    ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg'
                    : 'hover:bg-white/80 text-gray-700'
                  }`}
              >
                <div className={`p-2 rounded-lg transition-all duration-300 ${isActive(item.path)
                    ? 'bg-white/20'
        ...
        <div className="bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-200 p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                <div className="w-10 h-10 rounded-full overflow-hidden bg-gradient-to-r from-blue-600 to-purple-600 p-0.5 hover:shadow-lg hover:scale-105 transition-all duration-300">
