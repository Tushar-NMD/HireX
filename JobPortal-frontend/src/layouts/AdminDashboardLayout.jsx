...
const AdminDashboardLayout = ({ children }) => {
  ...
  return (
    <div className="home-page min-h-screen relative overflow-x-hidden">
      ...
      {/* Sidebar */}
      <div className={`fixed left-0 top-0 h-full w-80 bg-black/40 backdrop-blur-md border-r border-white/10 shadow-2xl transform transition-transform duration-300 z-50 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
        } lg:translate-x-0`}>
          ...
          {sidebarItems.map((item, index) => {
            ...
            return (
              <Link
                ...
                to={item.path}
                onClick={() => setIsSidebarOpen(false)}
                className={`group flex items-center space-x-3 p-4 rounded-xl transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${active
                ...
                <div className="w-10 h-10 rounded-full overflow-hidden bg-gradient-to-r from-amber-500 to-amber-700 p-0.5 hover:shadow-lg hover:scale-105 transition-all duration-300">
                  <div className="w-full h-full rounded-full overflow-hidden bg-black flex items-center justify-center">
                    {userProfilePic ? (
                      ...
                      <FaUserShield className="w-5 h-5 text-amber-500" />
                    )}
                  </div>
                </div>
